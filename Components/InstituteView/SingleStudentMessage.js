...
function SingleStudentMessage({item,index}) {
          ...
          <View style={[styles.item, styles.itemOut]}>
            <View style={[styles.balloon, {backgroundColor: '#1084ff'}]}>
              ...
              <Image
                styleName="small"
                borderRadius={5}
                source={{ uri: imageProvider(obj.imageLink)}}
  ...
  return (
    <>
      ...
      {messageRight(item.message)}
       {item?.images?.map((item)=>( 
            messageImages(item) 
       ))}

      {item.reply?messageLeft(item.reply):null}
...
export default SingleStudentMessage